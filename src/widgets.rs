//! Built-in node widgets for common rendering scenarios.
//!
//! This module contains [`ImageWidget`], which renders a texture-backed node
//! with a header label. Pass an `ImageWidget` to [`FlowCanvas`] when creating a
//! canvas to use it for rendering `String` nodes.

use crate::{FlowConfig, Node, NodeWidget, Transform};
use egui::{Vec2, epaint};

/// A [`NodeWidget`] that displays an egui texture below the node's label.
///
/// `size` should contain the texture's original dimensions. It is used to
/// preserve the texture's aspect ratio while the image is fitted into the
/// node's available area. The node's rendered size still comes from its
/// `width`/`height` values or the [`FlowConfig`] defaults.
///
/// # Usage
///
/// ```rust,ignore
/// let widget = ImageWidget::new(texture, egui::vec2(1200.0, 1600.0));
/// let events = FlowCanvas::new(&mut state, &widget).show(ui);
/// ```
...
impl ImageWidget {
    /// Creates an image widget from an egui texture and its original size.
...
impl NodeWidget<String> for ImageWidget {
    ...
    fn show(
        ...
        // Reserve the scale factor for consistent sizing of node elements
        let scale = transform.scale;

        // Border and Rounded Corners for the Node
        let corner_radius = config.node_corner_radius * scale;
        ...
        painter.rect_filled(
            screen_rect.translate(Vec2::new(6.0 * scale, 6.0 * scale)),
            corner_radius + 2.0,
